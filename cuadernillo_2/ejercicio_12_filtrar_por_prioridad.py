"""Cuadernillo II - Ejercicio 12: Filtrar objetos por atributo."""
...
def tickets_por_prioridad(tickets: list[Ticket], prioridad: str) -> list[Ticket]:
    prioridad = prioridad.strip().upper()
    return [ticket for ticket in tickets if ticket.prioridad == prioridad]
...
def main() -> None:
    tickets = [
        ...
        Ticket(3, "Carla Diaz", "ALTA"),
    ]

    altos = tickets_por_prioridad(tickets, "ALTA")
    for ticket in altos:
        print(ticket)
